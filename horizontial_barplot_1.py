...
# Step 1: Aggregate data by favorite item category and customer term
category_data = df.groupby(['favorite_item_category_name', 'customer_term'])['favorite_item_spend_dollars'].sum().reset_index()

# Step 2: Identify top 5 favorite item categories based on total spending
top_categories = category_data.groupby('favorite_item_category_name')['favorite_item_spend_dollars'].sum().nlargest(5).index
...
plt.savefig('graphs/slide_4.png', facecolor='#000000')
